...
def train(model, preloaded_batches, writer, lr = 0.001, len_print = 100, use_base = True, alpha = 0.9, check_params = False):
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'Saved models/' + str(alpha) + datetime.now().strftime(("%Y_%m_%d %H_%M_%S")) + str(mean_cost) +'best_model.pth')
        print(f"Best model saved with mean cost: {best_mean_cost:.2f}")
    writer.close()
    torch.save(model.state_dict(), 'Saved models/' + str(alpha) + datetime.now().strftime(("%Y_%m_%d %H_%M_%S")) + str(mean_cost) + 'last_model.pth')
